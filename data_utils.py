...
@torch.no_grad()
def evaluate_detect(model, dataset_ind, dataset_ood, return_score=False, visualize=True, dataname=None, ood_type=None):
    model.eval()
    test_ind_score = model.detect(dataset_ind, dataset_ind.splits['test'])
    
    
    dataset_ood = dataset_ood if isinstance(dataset_ood, list) else [dataset_ood]
    if isinstance(dataset_ood, list):
        result = []
        for i, d in enumerate(dataset_ood):

...
class Discriminator(nn.Module):
    ...
    def forward(self, c, h_pl, h_mi):
        ...
        sc_1 = self.sim(h_pl, c_x) 
        sc_2 = self.sim(h_mi, c_x) 
...
class GCRL(nn.Module):
    def __init__(self, in_channels, config):
        ...
        self.conv = GCNEncoder(in_channels, config)
       
        #self.conv = GATEncoder(in_channels, config)
     
        #self.conv = SAGEEncoder(in_channels, config)
